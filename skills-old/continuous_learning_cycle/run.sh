...
def _run_yolo_inference(image_path, model_path, threshold):
    """Run YOLO inference using available methods"""
    try:
        # Try our CVI TDL skill first
        tdll_result = _try_cvi_tdl_yolo(image_path, model_path, threshold)
        if tdll_result.get("status") == "success":
            return tdll_result
            
        # Fallback to direct maix.nn attempt (might SIGSEGV but we'll try in subprocess)
        # Actually, let's not risk the SIGSEGV and just report what we know
        return {
            "status": "deferred",
            "message": "YOLO inference deferred - CVI TDL implementation needed",
            "recommendation": "Implement CVI TDL YOLO inference for production use",
            "fallback_available": "maix.nn (known to SIGSEGV with current setup)"
        }
